...
const charElem = document.querySelector("[data-character]")


const jump_Speed = .45
const gravity = .002
const charFrameCount = 2
const frameTime = 100
...
function setupChar() {  //die Startkriterien für Character
    isJumping = false
    charFrame = 0
    currentFrameTime = 0
    yVelocity = 0
    setCustomProperty(charElem, "--bottom", 2)
    document.removeEventListener("keydown", onJump)
    document.addEventListener("keydown", onJump)
}

function updateChar(delta, speedScale) { //funktion damit character durchgehend udpated
    handleRun(delta, speedScale)
    handleJump(delta)
}

function handleRun() { //in updateChar permanent aufgerufen
    if (isJumping) { // falls er springt, wird die rennanimation nicht durchgeführt
        charElem.src = '../images/Kartoffel_stehend.svg'
        return
    }

    if (currentFrameTime >= frameTime) {
        charFrame = (charFrame + 1) % charFrameCount //charFrameCount = 2, d.h. % ergibt immer 0 oder 1, damit sich bild ändert
        if (charFrame === 1) {
            charElem.src = '../images/Kartoffel_gehend.svg'
        } else if (charFrame === 0) {
            charElem.src = '../images/Kartoffel_stehend.svg'
        }
...
function handleJump(delta) {  //in updateChar permanent aufgerufen
    if (!isJumping) return //falls isJumping = false, wird handleJump nicht durchgeführt

    incrementCustomProperty(charElem, "--bottom", yVelocity * delta) // der bottom wert wird erhöht um yVelocity * delta
    
    if (getCustomProperty (charElem, "--bottom") <= 2) { // damit der character nicht weiter runter fällt
        setCustomProperty(charElem, "--bottom", 2)
        isJumping = false
    }

    yVelocity -= gravity * delta // reduziert yvelocity um gravity * delta d.h. der char bewegt sich immer langsamer hoch und dann runter
}
...
function getCharRect() {
    return charElem.getBoundingClientRect() // gibt die Werte des Rectangle von Char wieder (also left, right, bottom, top)
}

function setCharLose() {
    charElem.src = "../images/Kartoffel_tot.svg"
}
...
function checkLose() {
    ...
    return getCactusRects().some(rect => isCollision(rect, charRect)) //schaut ob irgendeins der CactusRects mit dem CharRect übereinstimmen
}

function isCollision(rect1, rect2) {
    return rect1.left < rect2.right // falls cactus rect1 links von char rect2 ist
    && rect1.top < rect2.bottom // falls cactus rect1 top über char rect2 bottom
    && rect1.right > rect2.left // wenn cactus rect1 right rechts von char rect2 links ist
    && rect1.bottom > rect2.top
    
}

function handleLose() {
    setCharLose() //ändert das bild des characters
